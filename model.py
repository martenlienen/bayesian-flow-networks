...
class CtsBayesianFlowLoss(Loss):
    def __init__(
        ...
        distribution_factory: Union[
            CtsDistributionFactory, DiscreteDistributionFactory
        ],
    ...
    def discrete_time_loss(
        ...
        if hasattr(output_dist, "probs"):  # output distribution is discretized normal
            ...
            receiver_mix_dist = D.Categorical(
                probs=receiver_mix_wts, validate_args=False
            )
            receiver_components = D.Normal(
                output_dist.class_centres,
                (1.0 / alpha.sqrt()).unsqueeze(-1),
                validate_args=False,
            )
            receiver_dist = D.MixtureSameFamily(
                receiver_mix_dist, receiver_components, validate_args=False
            )
    ...
    def reconstruction_loss(
        ...
        if hasattr(output_dist, "probs"):  # output distribution is discretized normal
            ...
            final_dist = DiscretizedCtsDistribution(
                final_dist, num_bins, device=t.device, batch_dims=mean.ndim - 1
            )
