...
def setup(cfg) -> Tuple[nn.Module, dict, optim.Optimizer]:
    ...
    if "weight_decay" in cfg.optimizer.keys() and hasattr(
        model.net, "get_optim_groups"
    ):
...
def validate(
    ...
    val_id = pbar.add_task(
        "Validating",
        visible=True,
        total=cfg.val_repeats * max_steps,
        transient=True,
        loss=math.nan,
    )
    ...
    if checkpoint_root_dir is not None and (
        loss < best_val_loss or math.isinf(best_val_loss)
    ):
        ...
        checkpoint_training_state(
            step_checkpoint_path, accelerator, ema_model, step, run_id
        )
...
def train(
    ...
    run_id = "BFN" if isinstance(run, defaultdict) else run["sys"]["id"].fetch()
    train_id = pbar.add_task(
        f"Training {run_id}",
        start=cfg.start_step,
        total=cfg.n_training_steps,
        loss=math.nan,
    )
    checkpoint_root_dir = (
        init_checkpointing(cfg.checkpoint_dir, run_id) if is_main else None
    )
    ...
    with pbar:
        for step in range(cfg.start_step, cfg.n_training_steps + 1):
            ...
            if is_main and (step % cfg.checkpoint_interval == 0):
                checkpoint_training_state(
                    checkpoint_root_dir / "last", accelerator, ema_model, step, run_id
                )
                run["checkpoints/last"].track_files(str(checkpoint_root_dir / "last"))

            log(
                run["metrics"]["train"]["loss"],
                step_loss / cfg.accumulate,
                step,
                is_main and step % cfg.log_interval == 0,
            )
            log(
                run["metrics"]["epoch"],
                step // len(dataloaders["train"]),
                step,
                is_main,
            )
...
def main(cfg):
    ...
    logger.info(
        f"Seeded everything with seed {cfg.training.seed}", main_process_only=True
    )
    ...
    ema = (
        copy.deepcopy(model)
        if acc.is_main_process and cfg.training.ema_decay > 0
        else None
    )  # EMA on main proc only
    model, optimizer, dataloaders["train"] = acc.prepare(
        ...
        try:
            if cfg.meta.neptune:
                ...
                run = neptune.init_run(
                    project=cfg.meta.neptune, mode="debug" if cfg.meta.debug else None
                )
                run["accelerate"] = dict(
                    amp=acc.mixed_precision, nproc=acc.num_processes
                )
                log_cfg(cfg, run)
        except ImportError:
