...
def float_to_idx(flt: np.ndarray, num_bins: int):
    ...
    return torch.clamp(
        torch.floor(flt_zero_one * num_bins), min=0, max=num_bins - 1
    ).long()
...
def pe_encode(sequence_length: int, embedding_size: int) -> Tensor:
    ...
    pe[:, 0::2] = torch.sin(
        ...
        / torch.pow(
            1000,
            torch.arange(0, embedding_size, 2, dtype=torch.float32) / embedding_size,
        )
    )
    pe[:, 1::2] = torch.cos(
        pos
        / torch.pow(
            1000,
            torch.arange(1, embedding_size, 2, dtype=torch.float32) / embedding_size,
        )
    )
...
def pe_encode_float(x: Tensor, max_freq: float, embedding_size: int) -> Tensor:
    ...
    pe[..., 0::2] = torch.sin(
        ...
        / torch.pow(
            10000,
            torch.arange(0, embedding_size, 2, dtype=torch.float32, device=x.device)
            / embedding_size,
        )
    )
    pe[..., 1::2] = torch.cos(
        ...
        / torch.pow(
            10000,
            torch.arange(1, embedding_size, 2, dtype=torch.float32, device=x.device)
            / embedding_size,
        )
    )
    return pe
